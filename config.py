...
SCALP = dict(
    ...
    m1_confirm_half_bar = False, # True=半足モード: close>open で確認（バー途中でエントリー）
                                # False=通常モード: close>prev_close で確認（バー確定後・勝率重視）
...
WHIPSAW = dict(
    ...
    ratio_thr          = 5.0,   # ATR合計÷実効レンジ がこの値以上でウィップソー判定（旧2.0）
