...
def _find_all_watchdog_procs() -> list:
    ...
    try:
        for p in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                cl_str  = ' '.join(cl_list)
                if my_script in cl_str and '--watch' in cl_str:
...
def _make_restart_helper(pids: list, cmds: list) -> str:
    """再起動ヘルパースクリプトの内容を文字列で返す（一時ファイルに書き出して使用）。
    ヘルパーは DETACHED_PROCESS で起動され、親プロセス終了後も動作して自己削除する。
    """
    return f'''\
    ...
    class _MonitorBot(_discord.Client):
        ...
        async def _dispatch(self, cmd: str, args: list[str]) -> list[str]:
            ...
            if cmd == 'pull':
                ...
                # 1. git pull（stale ref エラー時は prune してリトライ）
                def _do_pull():
                    def _run(args):
                ...
                # 2. 全ウォッチドッグを検索
                watchdog_procs = _find_all_watchdog_procs()
                ...
                # 3. ヘルパースクリプトを書き出して DETACHED_PROCESS で起動
                ...
                try:
                    with open(helper_path, 'w', encoding='utf-8') as _hf:
                        _hf.write(_make_restart_helper(w_pids, w_cmds))
