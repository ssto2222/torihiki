...
| **ネックライン再テスト** | H1パターンのネックラインブレイク確認後、リテスト（戻り）でエントリー |
| **エリオット波動 Wave2** | M5足でW1+W2パターンを検知しWave3への参入（Fib 38.2〜78.6% 押し目/戻り） |
| **ボリュームブレイクアウト** | RVOL急増 + ローソク実体比 + RSI方向確認で SMA20 待ちをスキップ |
| **TTM スクイーズ** | BB×ケルトナー収縮からの解放方向にブレイクアウトエントリー |
| **MA クロス(SMA80×SMA200)** | M1 SMA80×SMA200 のクロスで方向を準備し、SMA20 タッチで執行 |
| **MTF フィルタ** | M1/M5/M15 SMA20 傾き + D1 SMA20 方向 + H1 レジームで多段階フィルタリング |
| **リスク管理** | ATRベース TP/SL / 証拠金維持率チェック / ロット自動調整 / マクロバイアス補正 |
| **N連勝後クールダウン** | 5連勝直後は4時間エントリーを控える（勝ちすぎ後の過信エントリー回避） |
| **エントリーログ・パフォーマンスレポート** | 発火エントリーをJSONLに記録し、シグナル種別ごとの勝率・損益を定期的にDiscordへ通知 |
...
### パス 2：ネックライン再テスト（直接エントリー）

H1パターンのネックラインを2本以上連続でブレイクした後、価格が一度離れてからネックライン付近に戻ってきた（リテスト）タイミングでエントリー。

```
アーミング: H1終値が確定バーで連続 ≥ nl_retest_min_bars(2)本 ネックラインを超えている
            信頼度 ≥ nl_retest_conf_min(0.45)
執行条件:   |close - ネックライン| ≤ ATR × nl_retest_margin_atr(0.5)
無効化:     価格がネックラインの逆側に ATR × nl_retest_inval_atr(2.0) 以上離れた（ブレイク失敗）
失効:       アーミングから nl_retest_expire_h(48) 時間
SL:         パターン極値（最安値/最高値）− ATR × nl_retest_sl_atr(0.5)
TP:         パターン測定値ターゲット
重複防止:   (パターン名, round(neckline, 0)) をフィンガープリントとして記録
フラグ:     _direct_confirmed = True、_is_nl_retest = True
```

### パス 3：Elliott Wave 2 エントリー（直接エントリー）
...
### パス 5：TTM スクイーズ（直接エントリー）

BB（ボリンジャーバンド）×ケルトナーチャネルの収縮（スクイーズ）が一定期間続いた後、解放（ブレイクアウト）した方向にエントリー。

```
条件:
  - squeeze_bars ≥ ttm_squeeze_min_bars(3)（収縮継続バー数）
  - 解放方向 = up: RSI ≥ ttm_rsi_buy_min(45.0)
  - 解放方向 = down: RSI ≤ ttm_rsi_sell_max(55.0)
  - 同バーで既に発火済みでないこと

TP: 通常 TP × ttm_tp_multi(1.6)
SL: 通常 SL × ttm_sl_multi(0.9)
フラグ: _is_ttm_signal = True（M1 BB/RSI 極端値ゲートを免除。
        スクイーズ解放はバンド際への急伸を伴うため、通常時の
        「伸び切り回避」ゲートと矛盾するため）
```

### パス 6：MAクロス(M1 SMA80×SMA200)準備 + SMA20タッチ（直接エントリー）

M1 の SMA80 と SMA200 のクロスで方向を「準備」状態にし、その後 M1 SMA20 にタッチしたタイミングで準備期間中 1 回だけ執行します。

```
準備:   SMA80 > SMA200 → BUY準備 / SMA80 < SMA200 → SELL準備
        逆クロスでこの関係が反転するまで準備状態を維持（次の逆クロスまで有効）
執行:   準備中に |close_M1 - SMA20_M1| ≤ ATR × ma_cross_sma20_margin_atr(0.3)
        → 準備期間中 1 回のみ発火（ma_cross_fired フラグで制御）
フラグ: _is_ma_cross_signal = True（RSI/BB極端値ゲート・逆トレンドゲートを免除。
        TTM/EW2 と同様、独立シグナルのため）
```

### パス 7：M1 早期実行（SMA20 タッチ待ち開始）
...
### パス 8：M5 RSI クロス → SMA20 タッチ → M1 確認
...
[Step 4] M1 確認バー（1本）
  m1_confirm_half_bar = True  → BUY: close_M1 > open_M1（バー途中で確認、早いがノイジー）
  m1_confirm_half_bar = False → BUY: close_M1 > prev_close（バー確定後に確認、既定値・勝率重視）
  SELL は対称（< open_M1 / < prev_close）
  共通条件: close_M1 が SMA20_M1 を割らない方向であること
  カウントが同一バーで重複しない
...
| 2 | **N連勝後クールダウン** | 直近 win_streak_count(5) 件が全勝 → win_streak_cooldown_h(4) 時間停止 | 適用 | 適用 |
| 3 | M1 RSI 過熱抑制 | RSI>65 で追加 BUY（既存ポジあり） | 適用 | 適用 |
| 4 | M1 RSI 売られすぎ抑制 | RSI<35 で追加 SELL（既存ポジあり） | 適用 | 適用 |
| 5 | **M1 SMA20 絶対ゲート** | M1 SMA20 下落中 → BUY禁止（逆も対称） | **免除** | 適用 |
| 6 | **M5 SMA20 価格位置** | close < SMA20_M5 → BUY禁止（逆も対称） | **免除** | **免除** |
| 7 | **D1 SMA20 方向** | D1 SMA20 下落中 → BUY禁止（逆も対称） | **免除** | 適用 |
| 8 | M5/M15 コンセンサス | M5 と M15 が両方とも逆方向傾斜 | **免除** | 適用 |
| 9 | RSI ハードゲート | RSI < 40.0 → BUY禁止 / RSI > 60.0 → SELL禁止 | **免除** | 適用 |
| 10 | M5 レジーム逆張り禁止 | M5 trend_up で SELL / trend_down で BUY | **免除** | **免除** |
| 11 | 禁止時間帯 | UTC 21 時台 | 適用 | 適用 |
| 12 | クールダウン | N 回ごとに cooldown_min 分待機 | 適用 | 適用 |
| 13 | ポジション上限 | available_slots ≤ 0（反対方向があればヘッジ許可） | 適用 | 適用 |

ウィップソーブロックと N連勝後クールダウンは「シグナル品質」ゲートではなくリスク管理のサーキットブレーカーのため、TTM/EW2/MAクロスを含む**全シグナル種別に無条件で適用**されます（免除なし）。
...
MT5 約定履歴（`history_deals_get`）から直近 `win_streak_count`(既定5) 件のクローズトレードを判定し、全勝だった場合は `win_streak_cooldown_h`(既定4) 時間エントリーを停止します（勝ちすぎ後の過信エントリーを避けるサーキットブレーカー）。同一の連勝で重複して再発火しないよう、トリガーとなった最新クローズ約定の時刻を記録します。`win_streak_count = 0` で無効化できます。

---

## エントリーログ・パフォーマンスレポート

### エントリーログ（`logs/entries_{symbol}.jsonl`）

発火してエントリーに至った全シグナル（`action='buy'/'sell'`）を1行JSON（JSONL）で追記します。記録項目: タイムスタンプ・symbol・action・signal_type・価格（close/SL/TP）・ロット・ATR・RSI（M5/M1）・レジーム（M5/H1）・各SMA値 など。後からシグナル種別ごとの勝率分析に使用します。

### パフォーマンスレポート（Discord 定期通知）

`perf_report_interval_h`(既定24時間) ごとに、直近 `perf_report_lookback_h`(既定24時間) のエントリーログと MT5 約定履歴（`position_id` で開始/決済を突合）を集計し、シグナル種別ごとの発火件数・決済済件数・勝率・損益を Discord へ通知します。決済済件数に対する勝率が `perf_report_min_winrate`(既定0.40) 未満かつ損益がマイナスの種別には「戦略見直しを推奨」と提案を添えます。対象エントリーが無い期間は通知をスキップします。
...
SCALP = dict(
    ...
    m1_confirm_half_bar = False,      # True=半足モード(close>open・バー途中) / False=通常モード(close>prev_close・バー確定後・勝率重視)
    ttm_squeeze_enabled = True,       # TTMスクイーズ有効化
    ttm_squeeze_min_bars = 3,         # スクイーズ継続最小バー数
    ttm_rsi_buy_min     = 45.0,       # TTM BUY の最低 RSI
    ttm_rsi_sell_max    = 55.0,       # TTM SELL の最高 RSI
    ttm_tp_multi        = 1.6,        # TTM TP 倍率
    ttm_sl_multi        = 0.9,        # TTM SL 倍率
    ma_cross_signal_enabled  = True,  # MAクロス(SMA80×SMA200)シグナル有効化
    ma_cross_sma20_margin_atr = 0.3,  # 執行マージン = ATR × この値
    nl_retest_conf_min  = 0.45,       # ネックライン再テスト アーミング最低信頼度
    nl_retest_min_bars  = 2,          # ブレイク確定に必要な連続バー数
    nl_retest_margin_atr = 0.5,       # 再テスト執行マージン = ATR × この値
    nl_retest_inval_atr = 2.0,        # 無効化（逆行）判定 = ATR × この値
    nl_retest_sl_atr    = 0.5,        # SL バッファ = ATR × この値
    nl_retest_expire_h  = 48,         # アーミング失効時間
    perf_report_enabled     = True,   # パフォーマンスレポート有効化
    perf_report_interval_h  = 24,     # レポート送信間隔（時間）
    perf_report_lookback_h  = 24,     # 集計対象期間（時間）
    perf_report_min_winrate = 0.40,   # この勝率未満かつ損益マイナスなら見直しを提案
)
```
...
```python
RULES_RISK = dict(
    max_consecutive_losses   = 10,    # 連続損失上限（超えたら長時間クールダウン）
    cooldown_large_loss_min  = 1440,  # 大損失後クールダウン（分）
    large_loss_threshold_usd = -10000,  # 大損失とみなす損益閾値（USD）
