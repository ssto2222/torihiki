...
**新機能 (2026-05): H1 RSI レベル別 TP 倍率制御**
- RSI >= 70（強い上昇）: TP幅 2.0×ATR（早期利確）
- 50 <= RSI < 70（通常）: TP幅 2.5×ATR
- RSI < 50（弱いトレンド）: TP幅 3.0×ATR（広いTP）
- トレンド終焉が近い高RSI領域での利確を加速化
- 1H BB2σ 近傍時は初期 TP を 2σ に抑え、トレーリングで利益を伸ばす
...
`config.py` ではトレードルールを以下のように分類しています。

- `RULES_GENERAL`
  - 資金管理の全体制限
  - `total_risk_pct` で最大ポジション数を間接管理
- `RULES_ENTRY`
  - エントリー判定のスコア閾値
  - `min_score` がエントリー可否に直結
- `RULES_RISK`
  - 連続損失停止や大損失後のクールダウン
  - `max_consecutive_losses`, `cooldown_large_loss_min`, `large_loss_threshold_usd`
- `RULES_EXIT`
  - 最低保有時間などエグジット関連の補助設定
  - `min_hold_minutes`

この構造は `RULES` としてフラットに統合されるため、既存コードの参照はそのまま動作します。
...
- **H1 RSI レベル別TP制御**: RSI70以上で早期利確、低いRSIで広いTP
  - RSI >= 70: TP = 2.0×ATR（強いトレンド、トレンド終焉回避）
  - 50 <= RSI < 70: TP = 2.5×ATR（通常）
  - RSI < 50: TP = 3.0×ATR（弱いトレンド、リスク・リワード重視）
