...
CHECK_INTERVAL = 0.5  # スキャル用に0.5秒間隔


def check_pause_signal():
    """毎ループ実行：スマホからのBuy Stop(Magic=0)を確認"""
    ...
    if has_stop_order:
        if not os.path.exists(FLAG_FILE):
            with open(FLAG_FILE, "w") as f: f.write("paused")
...
_scalp_buy_confirm_pending: bool     = False    # BUY SMA20 タッチ確認待ち
_scalp_buy_confirm_at: datetime | None = None    # BUY 確認開始時刻
_scalp_buy_confirm_count: int        = 0        # BUY 上昇バー確定本数
_scalp_buy_confirm_bar_time: object  = None     # BUY 最後にカウントしたバー時刻
_scalp_buy_confirm_level: float      = 0.0      # BUY 確認中のRSIレベル
_scalp_sell_confirm_pending: bool    = False    # SELL SMA20 タッチ確認待ち
_scalp_sell_confirm_at: datetime | None = None  # SELL 確認開始時刻
...
# ── SMA20 タッチマージンキャッシュ（シンボル別、分析で自動設定）──────
_sma20_touch_margins: dict = {}   # {'BTCUSD': 10.5, 'XAUUSD': 2.3, ...}
...
def _analyze_sma20_touch_margin(symbol: str, cfg: dict) -> float:
    ...
    gap = SMA20_at_signal - max_close_in_next_30_M1_bars
      gap > 0 → SMA20 未達（その値だけ下にいる）
      gap ≤ 0 → SMA20 以上まで上昇（余裕でタッチ）
    touch_margin = percentile(gaps, pct)  → pct% のケースでタッチ判定が成立
    """
    try:
...
_scalp_sell_sma_pending, _scalp_sell_sma_at, _scalp_sell_sma_level, \
    ...
    try:
        ...
        if not buy_enabled and _scalp_buy_confirm_pending:
        ...
        scalp_lot_max = (float(_lot_max_cfg.get(symbol, float('inf')))
                         ...
                         else float(_lot_max_cfg))  # 数値指定の後方互換
        ...
        if confirmed_signal is None and _scalp_buy_confirm_pending:
            timeout_min = 30
            if (_scalp_buy_confirm_at is not None and
                    (now - _scalp_buy_confirm_at).total_seconds() > timeout_min * 60):
                _scalp_buy_confirm_pending  = False
                _scalp_buy_confirm_at       = None
                _scalp_buy_confirm_count    = 0
                _scalp_buy_confirm_bar_time = None
                ...
                if is_up_bar and m1_bar_cur != _scalp_buy_confirm_bar_time:
                    _scalp_buy_confirm_count   += 1
                    _scalp_buy_confirm_bar_time = m1_bar_cur
                elif not is_up_bar:
                    _scalp_buy_confirm_count    = 0
                    _scalp_buy_confirm_bar_time = None

                if _scalp_buy_confirm_count >= 2:
                    confirmed_signal            = 'buy'
                    crossed_level               = _scalp_buy_confirm_level
                    _scalp_buy_confirm_pending  = False
                    _scalp_buy_confirm_at       = None
                    _scalp_buy_confirm_count    = 0
                    _scalp_buy_confirm_bar_time = None
...
def run_bridge(cfg: dict, once: bool = False, mode: str = 'normal'):
    ...
    # スキャルプモード: SMA20 タッチマージンを分析・ロード（MT5 接続後に実行）
    if mode == 'scalp':
        _load_sma20_touch_margins([symbol], cfg)
        m = _sma20_touch_margins.get(symbol)
        if m is not None:
    ...
    try:
        ...
        while True:
            ...
            if data:
                ...
                # ── Discord 通知: アクション状態変化時のみ ──────────────
                curr_action = data.get('action', 'none')
                if curr_action != _last_discord_action[0]:
                    try:
...
def main():
    ...
    print(f"メイン起動: {SYMBOL}")
    send_discord("【システム】自動売買を開始しました。")
    ...
    try:
        while True:
           # ターンの最初に必ずチェック
            ...
            # 2. 定期生存報告 (1時間に1回)
            curr = time.time()
            if curr - LAST_NOTIFY_TIME > NOTIFY_INTERVAL:
                send_discord(f"✅ **【生存報告】** {SYMBOL} は正常に稼働中です。")
                LAST_NOTIFY_TIME = curr
